...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item \textbf{主要方法}：基于大语言模型的提示词工程，将政策文本映射为结构化权利与执行信息。
...
\begin{frame}
    \frametitle{Pipeline 设计}
    \small
    \begin{itemize}
        \item 本次实验沿用上次已跑通的句级处理流水线（切句、聚类、送标、统计）。
        \item 本次主要改动集中在三点：\textbf{Prompt}、\textbf{输出结构}、\textbf{few-shot}。
    \end{itemize}
    \vspace{0.2em}
    \centering
    \begin{tikzpicture}[
...
\begin{frame}
    ...
    \small
    \begin{itemize}
        \item 23 款应用，共 9639 句；累计送标候选 4371 条。
    \end{itemize}
    \centering
